...
2. **Get the gates table.** Read `growth-engine/.state/gate-state.md`, the computed state described in `../../references/state.md`. It holds one row per gate item for this founder's track, each with its state and the evidence. Do not count pieces, rows, people or approvals yourself, and do not ask the `status-checker` agent to count them again.
...
| Operations | Bottleneck, the pack to publish first, its copy | After Session 2 | Gate C, Session 3, then loaded at the clinic |
| GoHighLevel values | Every value the snapshot arrives without, written in `engines/ops/ghl-values.md` | Before the clinic, usually the evening after Session 3 | Pasted in at the clinic, before anything is published |
...
Every date comes from the cohort block in `../../references/gates.md`. Name a session by its day and date while it is still ahead. Once a date there has passed, use relative words instead: "your third session", "the clinic", "the Saturday of your programme".
...
**B2C sends.** The 25 DMs go out by hand in Atlanta, on the Saturday in the cohort block. Before that day they are not due, and the computed state says `not due`: never list them as missing. From that day, report how many people are at `sent` or later.
...
Then name one plain thing they may not know they can say, whichever fits what just happened, once and in their own words: "save my work", "add a file", "set up my routines", or "help". One, not a list.
