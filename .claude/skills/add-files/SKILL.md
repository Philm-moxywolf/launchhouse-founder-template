...
3. Hand on:
   - **Writing samples, and the Brain's voice was built without them:** offer to refresh the voice section (`/growth-engine:brain`, update mode). If they say yes and content or engine 2 files already exist, those were written in the old voice: the "When one file changes, what goes stale" table in `../../references/contract.md` says which files those are. Name them, say they are now out of date, and offer to rebuild each one. Never rebuild quietly.
   - **Writing samples, and no content yet:** say the content engine will read them (`/growth-engine:content`).
4. Say once how they check where they stand: "where am I up to" lists every gate item, what is done and what is still open.
