...
**Order matters, and the writing comes first.** The words are written before the clinic, usually the evening after Session 3, from the Brain and the ops file. Nothing here needs the snapshot to exist, so never tell a founder they are early. The snapshot is loaded at the clinic, whose date is in the cohort block in `../../references/gates.md`, which brings the slots but not the words, and the words written earlier are pasted in then. Publishing comes after that.
...
At either stopping point, name the next step, and say that "where am I up to" shows where they stand.
