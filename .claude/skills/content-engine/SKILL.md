...
- **On B2C** it becomes what they watch go wrong, and the founder's own story where there is one. The story pillar only holds when this founder lived the problem themselves. If they did not, or if the business is run by more than one of them, say so and drop the story half: leave the pillar as what they see go wrong. Check the Brain's `Team` line before you decide, and ask if it does not say. A borrowed founder story is an invented one.
...
**If track is b2c, ask who is on camera** in the same turn, before the first batch is written. Is it them, somebody else who works with them, or nobody on camera at all? The Brain's `Team` line is the start of the answer, so read it first and ask only what it does not say. A founder who will not go on camera gets no talking-head scripts: write to the answer instead, with hands and work in shot, a voiceover, or text on screen. Never ask this of a B2B founder, whose 30 pieces are words.

Hold on to the answers. Step 4 uses the media one.
...
## When the words change after something was built from them

The thirty pieces are what the sheet, the ledger and the playbook insert were built from. Change a piece after those exist, and they no longer match. The "When one file changes, what goes stale" table in `../../references/contract.md` says which files those are.

Whenever a piece is rewritten, say in plain words which files are now out of date, and offer to rebuild each one. Never rebuild quietly. A rebuilt file is still read and approved by the founder, the same as the first time.
...
6. Say once, plainly, how they check where they stand at any point: "where am I up to" lists every gate item, what is done and what is still open.
