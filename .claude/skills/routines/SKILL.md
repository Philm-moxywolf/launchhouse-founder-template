...
| Countdown | `countdown.md` | Daily in the last days before Atlanta, 08:00 their time | `drafts/before-atlanta.md`: what is not done yet, gate by gate | both |
...
2. Convert the schedule to a time in the founder's timezone from `.state/profile.md`. Any programme date comes from the cohort block in `../../references/gates.md`. If those dates have passed, say so plainly, and leave the countdown out.
