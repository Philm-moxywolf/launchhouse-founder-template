...
Read `.claude/tool-packs/{{id}}/pack.md` and `.claude/tool-packs/{{id}}/knowledge.md` first, every time, before doing anything else.
...
- Never call a tool this pack's `policy.tsv` or `policy.local.tsv` denies, and never call a tool outside `.claude/tool-packs/{{id}}/inventory.txt`.
