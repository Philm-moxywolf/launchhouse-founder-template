...
Re-run ToolSearch to confirm the inventory still matches what is connected. Re-check every source above still resolves and still says what this file claims. Run `sh .claude/scripts/tool-packs.sh --validate {{id}}` and `sh .claude/scripts/tool-packs.sh --test {{id}}`. Update `verified_on` in `pack.md` once done.
