...
**The install comes first.** Before adding this connector in Claude, the founder opens GoHighLevel's own install link in a browser. That page is on LeadConnector's site, which is GoHighLevel's own company, so they sign in there with the same email and password they use for GoHighLevel. They pick their business's sub-account (never the agency), and approve. Without this step the sign-in window still finishes, but Claude never connects: no tools appear. The link:
...
(confirmed working with a founder on 22 September 2026). Its scope list asks for exactly the same permissions GoHighLevel's in-Claude sign-in asks for, so it grants nothing extra.
