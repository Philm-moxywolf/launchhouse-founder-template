...
3. Re-run the validator and the test suite once they exist in this repository (`sh .claude/scripts/skill-packs.sh --validate apollo`, `--compile`, `--test apollo`), or, until then, replay the `tests.tsv` cases directly against `.claude/scripts/mcp-guard.sh` the way `.claude/tests/run.sh` replays its own fixtures.
