...
Founders and their Claude edit `.claude/skill-packs/apollo/policy.local.tsv`, never `policy.tsv`, which is the shipped template file. A local row may only add a `deny` or `ask`, tighter than what is here; it can never loosen a row in `policy.tsv`, and `mcp-guard.sh` only ever applies the stricter of the two.
