...
#
# Two forms may run in the conversation, and neither ever shows the key:
#   --check    says whether the item is there and in the right shape
#   --connect  the same check, then, when all is right, writes .mcp.json at the
#              top of this folder so the app connects to GoHighLevel when it
#              next opens. .mcp.json holds this computer's own path to the
#              folder, never the key, and git ignores it.
