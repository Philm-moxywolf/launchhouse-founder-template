...
# A reply/trigger marker in the SENTENCE ITSELF (not just a nearby label).
# This is started_by_them()'s trigger language minus the bare, generic word
# "inbound" on its own (a heading like "## Inbound" says nothing about who
# wrote first), plus plain reply wording. A label or heading can only excuse
# a sentence when the sentence carries one of these markers on its own.
function reply_marker(lo) {
  return match(lo, /comment[ -](to|2)[ -]dm|(messag[a-z]*|contacted|wr[io]te|dm[a-z']*) you first|user[- ]initiated|started the conversation|triggered by|keyword trigger|trigger keyword|comment keyword|in (reply|response) to|repl[a-z]* to (their|the|a|every|each|any|incoming) (comment|message|dm|question|reply|story)|respond[a-z]* to (their|the|a|every|each|any|incoming)|who (comment|messag|dm|asked|replied|wrote|reply|replies)|opt(ed)?[- ]in|messaging window|24[- ]hour window|on (the|their|a|each|every|any) (comment|reply|message|dm)|(someone|somebody|a follower|they|people|anyone|customers|the person|a customer) (comment|comments|commented|messages|messaged|dms|replies|replied|writes|wrote|asks|asked)|commented first|(every|each|a) (new )?comment|comments? (with|containing|using) |when they comment|after they comment|the moment they comment|repl(y|ies|ied|ying)|respond(s|ed|ing)?|answer(s|ed|ing)?|thanks for|thank you for|sent when they|after they/)
}

function cold_span(lo,    s, off, p, before) {
  s = lo; off = 0
  while (match(s, /cold|unsolicited|outbound|new followers?|every follower|each follower|all (your )?followers|your list|the list|prospects|target accounts|target list|story viewers|post viewers|profile viewers|(people|users|everyone|accounts|anyone) who (view|like|liked|likes|follow|visit)|not spoken|haven't spoken|have not spoken|never spoken|in bulk|on your behalf|while you sleep|autopilot|set and forget|every (morning|day|night|week|month|hour|single day)|each (morning|day|night|week|month|hour)|(^|[^a-z])daily([^a-z]|$)|(^|[^a-z])nightly([^a-z]|$)|(^|[^a-z])hourly([^a-z]|$)|on a (set )?schedule|on schedule|on a timer/)) {
    p = off + RSTART
...
function dm_check(ln, sent, nextsent, ctx,    lo, c, d, dist, replaces, before, refusal, lc) {
  lo = tolower(sent)
  ...
  # The label above the message counts as part of it. Ordinary markdown puts a
  # heading or a bold label on its own line, a blank line, then the message, and
  # the blank line used to hide the label from this sentence. So an approved
  # inbound reply reads the same whether or not that blank line is there.
  # A label is never enough by itself: a heading like "## Inbound" says nothing
  # about who wrote first, so the SENTENCE must independently carry a
  # reply/trigger marker (reply_marker) for the label to excuse it at all. If
  # either the label or the sentence names cold, bulk or scheduled contact,
  # the hold stands regardless of what the label says.
  if (ctx != "") {
    lc = tolower(ctx)
    if (reply_marker(lo) && !cold_span(lc) && !cold_span(lo)) return
  }
...
# ------------------------------------------------------------- label context
# A heading, or a line that is only a bold label ("**Approved inbound reply**",
# with or without a trailing colon, with or without a list marker in front).
function is_heading(s) { return s ~ /^[[:space:]]*#+[[:space:]]/ }

function label_text(s,    t, rest) {
  t = s
  sub(/^[[:space:]]*([-*+>][[:space:]]+)?/, "", t)
  if (!match(t, /^(\*\*[^*]+\*\*|__[^_]+__)/)) return ""
...
END {
  ...
  # Masked copy for the other-track words. The same pass records, for every
  # line, the nearest heading and bold label above it, within 10 lines, so a
  # labelled block can be judged with its label.
  infence = 0
  hd = ""; hdln = 0; lb = ""; lbln = 0
  for (i = 1; i <= n; i++) {
    raw = lines[i]
    if (raw ~ /^[[:space:]]*(```|~~~)/) { infence = !infence; ctxline[i] = ""; continue }
    if (!infence) {
      if (is_heading(raw)) { hd = raw; hdln = i; lb = ""; lbln = 0 }
      else { lt = label_text(raw); if (lt != "") { lb = lt; lbln = i } }
    }
    ctxline[i] = ((hdln && i - hdln <= 10) ? hd : "") " " ((lbln && i - lbln <= 10) ? lb : "")
    style(i, raw)
    if (infence) continue
  ...
  for (u = 1; u <= nu; u++) {
    ...
    for (k = 1; k <= ns; k++) {
      ...
      dm_check(ustarts[u], sents[k], nxt, ctxline[ustarts[u]])
      delete sents[k]
    }
  }
